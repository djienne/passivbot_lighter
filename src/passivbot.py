...
class Passivbot(ExchangeInterface):
    ...
    async def start_bot(self):
        ...
        try:
            warmup_concurrency = 2 if self.exchange == "lighter" else 8
            await self.warmup_candles_staggered(concurrency=warmup_concurrency)
        except Exception as e:
            logging.info(f"warmup skipped due to: {e}")
        await asyncio.sleep(1)
        self._log_memory_snapshot()

        # Reset rate-limit backoff state accumulated during warmup burst.
        # If we hit 429s during warmup, wait for server-side window to clear
        # before resetting local state and starting data maintainers / execution.
        if hasattr(self, "_global_backoff_consecutive"):
            if self._global_backoff_consecutive > 0:
                cooldown = min(max(30, 15 * self._global_backoff_consecutive), 60)
                logging.info(
                    f"Resetting warmup rate-limit backoff "
                    f"(was {self._global_backoff_consecutive} consecutive); "
                    f"cooling down {cooldown}s for server window to clear"
                )
    ...
    async def update_pos_oos_pnls_ohlcvs(self) -> bool:
        ...
        if not positions_ok:
            return False
        open_orders_ok = await self.update_open_orders()
        if not open_orders_ok:
            return False
        # Only fetch PnLs when a fill was detected to reduce REST calls
        if getattr(self, "_fill_detected", False):
            pnls_ok = await self.update_pnls()
            if not pnls_ok:
                return False
        if self.stop_signal_received:
            return False
    ...
    def _check_minimum_balance(self):
        """Check if account balance is sufficient for the configured strategy.
        Raises RuntimeError if no symbol can meet minimum entry cost."""
        if not hasattr(self, "effective_min_cost") or not self.effective_min_cost:
            return
        symbols_to_check = list(self.effective_min_cost.keys())
        failures = []
        checked = 0
        for pside in ["long", "short"]:
            twel = self.bot_value(pside, "total_wallet_exposure_limit")
            n_pos = self.bot_value(pside, "n_positions")
            if twel <= 0 or n_pos <= 0:
                continue
            for symbol in symbols_to_check:
                checked += 1
                emc = self.effective_min_cost[symbol]
                we_limit = self.get_wallet_exposure_limit(pside, symbol)
                entry_pct = self.bp(pside, "entry_initial_qty_pct", symbol)
                affordable = self.balance * we_limit * entry_pct
                if affordable < emc:
                    denom = we_limit * entry_pct
                    required = emc / denom if denom > 0 else float("inf")
                    failures.append(
                        {
                            "symbol": symbol,
                            "pside": pside,
                            "effective_min_cost": emc,
        ...
        if failures and len(failures) >= checked:
            ...
            for f in failures:
                logging.error(
                    ...
                    f"need ${f['required_balance']:.2f}, have ${self.balance:.2f} "
            ...
            for f in failures:
                logging.warning(
                    f"Balance marginal for {f['symbol']} {f['pside']}: "
                    f"need ${f['required_balance']:.2f}, have ${self.balance:.2f}"
                )
    ...
    async def handle_balance_update(self, upd, source="WS"):
        ...
        try:
            bal_new = round_dynamic(upd[self.quote]["total"], 10)
            equity = bal_new + (await self.calc_upnl_sum())
            if abs(self.balance - bal_new) > 1.0:
                logging.info(
                    f"balance changed: {self.balance} -> {bal_new} equity: {equity:.4f} source: {source}"
                )
                self.execution_scheduled = True
            self.balance = max(bal_new, 1e-12)
        except Exception as e:
    ...
    async def calc_ideal_orders(self, allow_unstuck: bool = True):
        ...
        # Remove reduce_only orders below exchange minimum
        for symbol in ideal_orders_f:
            ideal_orders_f[symbol] = [
                o for o in ideal_orders_f[symbol]
                if not (o["reduce_only"] and abs(o["qty"]) < self.min_qtys.get(o["symbol"], 0))
            ]
